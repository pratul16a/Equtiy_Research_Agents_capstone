...
def load_documents(docs_dir: str | None = None) -> list:
    """Load documents from PDF and text files in the given directory.

    Args:
        docs_dir: Path to the directory containing documents.
                  Defaults to data/sample_reports/
    """
    if docs_dir is None:
    ...
    if not docs_path.exists():
        ...
        print(f"   Create it and add PDF/TXT files, then re-run ingestion.")
        return []
...
def create_faiss_index(chunks: list, index_path: str | None = None) -> None:
    ...
    Args:
        ...
        index_path: Where to save the FAISS index. Defaults to data/faiss_index/
    """
    if index_path is None:
        index_path = FAISS_INDEX_PATH

    if not chunks:
        print("⚠️  No chunks to index. Add documents to data/sample_reports/ first.")
        return
